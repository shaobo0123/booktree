<template>
  <section class="flex min-w-[280px] max-w-[380px] flex-1 flex-shrink-0 flex-col bg-white/80">
    <div class="flex h-14 items-center gap-2.5 border-b border-slate-100 px-4">
      ...
      <span class="flex h-5 items-center rounded-full bg-slate-100 px-2 text-[11px] font-medium text-slate-500">
        {{ nodes.length }}
      </span>
    </div>
...
function onDragEnd() {
  ...
  emit('reorder', props.nodes[0]?.parent_id ?? null, orderedIds);
}
</script>
